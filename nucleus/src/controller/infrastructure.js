...
export const createInfrastructure = async (req, res) => {
  try {
    ...
    // TODO: Check these
    const { name, infrastructureTemplateId, description, variables } = req.body;
    ...
    if (
      ...
      !variables.every((variable) => {
        if (variable.type !== "infrastructure-id")
          return variable.name && variable.value && variable.value !== "";
        return true;
      })
    ) {
      return res.status(400).json({ error: "error 'variables' is invalid" });
    }
